...
import MarkAsPaidButton from "@/app/components/MarkAsPaidButton"; // Unico bottone per i pagamenti
...
export default async function AdminPage() {
  const supabase = await createClient(); // Await per la versione async
  ...
  // Questa è l'unica lista che serve per i pagamenti manuali
  ...
  return (
    <div className="w-full space-y-10">
              ...
              {missionsToReview && missionsToReview.length > 0 ? (
                missionsToReview.map((item) => (
                  <tr key={`mission-${item.id}`} className="border-b border-gray-700 hover:bg-white/5">
                    ...
                    <td className="p-4 text-white font-semibold">{item.missions?.[0]?.title ?? 'N/A'}</td>
      ...
      <div>
        ...
        {/* Bottone Esportazione CSV RIMOSSO */}
